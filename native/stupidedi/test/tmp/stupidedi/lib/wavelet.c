...
stupidedi_wavelet_init(stupidedi_wavelet_t* w, stupidedi_packed_t* s, stupidedi_huffman_t* codec)
{
    ...
    /* n_ones[l]: The number of 1-bits at level l. The last level's count is
     * needed by rank/select (to reach symbol-level granularity, matching
     * `symbols`), even though access() never needs to look past it. */
    stupidedi_packed_t *n_ones, *rows, *symbols;
    n_ones  = stupidedi_packed_new(depth, nbits(length + 1));
    ...
    for (size_t l = 0; l < depth; ++l)
    {
        ...
        for (size_t i = 0, n0_ = 0, n1_ = 0; i < n_curr; ++i)
        {
            ...
            {
                /* Record where the last bit of this symbol is located,
                 * relative to the start of this row. NOTE: If these values
                 * were written in a separate pass (probably using depth
                 * rank calls per symbol), then we could discard length bits
                 * from s_next each iteration. For fixed-width encoded
                 * sequences, that does not reduce the peak memory usage at
                 * the first iteration though. It doesn't apply to
                 * variable-length encoded sequences, so it's not worth
                 * doing this.
                 *
                 * When a symbol repeats, every occurrence shares the same
                 * bit path and lands in a contiguous run here; we only want
                 * the *first* (smallest) position in that run, since that's
                 * what a rank/select walk starting fresh from position 0
                 * naturally computes -- so the counters still advance for
                 * every occurrence (later, distinct symbols need the
                 * advanced count), but symbols[c] itself is only written
                 * the first time this symbol is seen. */
                size_t pos = (w & mask) ? n1_++ : n1 + n0_++;
                if (stupidedi_packed_read(symbols, c) == length)
                    stupidedi_packed_write(symbols, c, pos);
            }
...
stupidedi_wavelet_deinit(stupidedi_wavelet_t* w)
{
    ...
    if (w->symbols) w->symbols = stupidedi_packed_free(w->symbols);
...
stupidedi_wavelet_access(const stupidedi_wavelet_t* wm, size_t i)
{
    ...
    for (; l < wm->depth; ++l)
    {
        ...
        {
            /* We're writing bits left-to-right. The 1-bin occupies this
             * row's leading positions and the 0-bin follows it (see the
             * second pass in stupidedi_wavelet_init), so descending into
             * the 0-child needs to skip past this level's 1-bin. */
            c = (c << 1) | 0;
            i = row_rank0(wm, l, i) + row_ones(wm, l + 1);
        }
        else
        {
            c = (c << 1) | 1;
...
stupidedi_wavelet_rank(const stupidedi_wavelet_t* wm, uint64_t c, size_t i)
{
    ...
    /* All `depth` routing decisions (levels 0..depth-1) are needed to reach
     * symbol-level granularity -- matching what `symbols[c]` records, since
     * that's written using the terminal level's own bit split too. */
    for (; l < depth; ++l)
    {
        if (w & mask)
        {
        ...
        {
            p = row_rank0(wm, l, p) + row_ones(wm, l + 1);
            i = row_rank0(wm, l, i) + row_ones(wm, l + 1);
        }
    ...
    /* symbols[c] only holds a meaningful position when c actually occurs in
     * the data -- when it doesn't, symbols[c] is the "not found" sentinel
     * (== length), which has no reason to equal the walk's own bucket-
     * boundary result p (p is still correct: i==p, giving rank 0). */
    assert(stupidedi_packed_read(wm->symbols, c) == wm->length ||
           p == stupidedi_packed_read(wm->symbols, c));
    return i - p;
}
...
stupidedi_wavelet_select(const stupidedi_wavelet_t* wm, size_t r, uint64_t c)
{
    ...
    /* `r` starts as a position within the last level's own row (matching
     * what rank's forward pass lands on after all `depth` transformations
     * through levels 0..depth-1). Walk those same levels in reverse, from
     * depth-1 down to 0, mirroring stupidedi_wavelet_rank. */
    for (size_t n = 0; n < depth; ++n)
    {
        size_t l = depth - 1 - n;
        size_t ones = row_ones(wm, l + 1);

        if (w & mask)
            r = row_select1(wm, l, r);
        else if (r < ones)
            /* r doesn't even reach past this level's 1-bin, so there's no
             * such occurrence -- avoid underflowing (r - ones). */
            return SIZE_MAX;
        else
            r = row_select0(wm, l, r - ones);
...
/* Number of 1-bits at level l-1. n_ones has `depth` entries (0..depth-1),
 * covering the last level too -- rank/select need it to reach symbol-level
 * granularity, matching `symbols`. */
static inline size_t
...
{
    size_t from, rank, pos;
    from = row_location(wm, l);
    rank = matrix_rank0(wm, l);
    pos  = stupidedi_rrr_select0(wm->matrix, rank + r);

    /* select0 operates on the whole matrix, so a target rank that doesn't
     * exist *within this row* can still resolve to a valid position in the
     * next row. Treat anything at or past this row's end as not found. */
    if (pos == SIZE_MAX || pos > row_location(wm, l + 1))
        return SIZE_MAX;
...
row_select1(const stupidedi_wavelet_t* wm, uint8_t l, size_t r)
{
    ...
    pos  = stupidedi_rrr_select1(wm->matrix, rank + r);

    /* select1 operates on the whole matrix, so a target rank that doesn't
     * exist *within this row* can still resolve to a valid position in the
     * next row. Treat anything at or past this row's end as not found. */
    if (pos == SIZE_MAX || pos > row_location(wm, l + 1))
        return SIZE_MAX;
