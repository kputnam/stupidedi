...
stupidedi_rrr_access(const stupidedi_rrr_t* rrr, size_t i)
{
    ...
    return (block & (UINT64_C(1) << i)) >> i;
}

/* Counts 1-bits in [0,i], i.e. *including* position i -- this is what the
 * marker/block/class arithmetic below actually computes. The public API
 * (rank0/rank1) documents the usual succinct-data-structure convention of
 * counting [0,i), i.e. *excluding* position i, so they're thin wrappers
 * around this that shift the index by one. */
static size_t
rank1_incl(const stupidedi_rrr_t* rrr, size_t i)
{
    assert(rrr != NULL);
...
stupidedi_rrr_rank1(const stupidedi_rrr_t* rrr, size_t i)
{
    ...
    return rank1_incl(rrr, i - 1);
}

size_t
stupidedi_rrr_rank0(const stupidedi_rrr_t* rrr, size_t i)
{
    return ((i < rrr->length) ? i : rrr->length) - stupidedi_rrr_rank1(rrr, i);
}

/* Returns the raw 0-indexed position of the r-th 0-bit (or SIZE_MAX). The
 * public API documents (and callers expect) the "shortest prefix length"
 * convention, i.e. one past this position -- see stupidedi_rrr_select0. */
static size_t
select0_pos(const stupidedi_rrr_t* rrr, size_t r)
{
    assert(rrr != NULL);
    ...
    /* Number of 0-bits in the not-yet-counted portion of this block. */
    size_t zeros_here = (rrr->block_size - twice) - popcount(block);

    if (r - rank <= zeros_here)
    {
        ...
        uint64_t zblock = ~block;
        if (rrr->block_size - twice < 64)
            zblock &= (UINT64_C(1) << (rrr->block_size - twice)) - 1;
...
{
    size_t pos = select0_pos(rrr, r);
    return pos == SIZE_MAX ? SIZE_MAX : pos + 1;
}

/* Returns the raw 0-indexed position of the r-th 1-bit (or SIZE_MAX). The
 * public API documents (and callers expect) the "shortest prefix length"
 * convention, i.e. one past this position -- see stupidedi_rrr_select1. */
static size_t
select1_pos(const stupidedi_rrr_t* rrr, size_t r)
{
    assert(rrr != NULL);
    ...
    if (r - rank <= (uint64_t)popcount(block))
    {
        /* The r-th 1-bit occurs somewhere before the next block. */
        for (i = -1; rank < r; ++rank)
        {
            i      = ctz(block);
            block &= ~(UINT64_C(1) << i);
    ...
    {
        /* Just go ahead and count all the unread 1-bits */
        rank      += popcount(block);
        offset_at += width;
        ++ class_at;
    }
...
decode_block(uint8_t block_size, uint8_t class, uint64_t offset)
{
    ...
    if (class > 0)
        value |= class < 64 ? (UINT64_C(1) << class) - 1 : UINT64_MAX;
...
stupidedi_rrr_builder_write(stupidedi_rrr_builder_t* rrrb, uint8_t width, uint64_t value)
{
    ...
    while (rrrb->block_need <= width)
    {
        ...
        mask   = rrrb->block_need < 64 ? (UINT64_C(1) << rrrb->block_need) - 1 : UINT64_MAX;
        block  = rrrb->block;
...
stupidedi_rrr_builder_to_rrr(stupidedi_rrr_builder_t* rrrb, stupidedi_rrr_t* rrr)
{
    ...
    if (rrrb->marker_at < rrr->nmarkers)
    {
        ...
        mask        = rrrb->marker_need < 64 ? (UINT64_C(1) << rrrb->marker_need) - 1 : UINT64_MAX;
