...
typedef struct stupidedi_huffman_wavelet_t
{
    ...
    /* O(σ log L), lengths[i]: Length of codeword for symbol i (symbols are
     * numbered ascending by frequency, per the caller's input) */
    stupidedi_wavelet_t*    lengths;
} stupidedi_huffman_wavelet_t;

/* Uncompressed representation of a codebook */
typedef struct stupidedi_huffman_packed_t
{
    /* O(σL), encode[i]:  The codeword for symbol i (symbols are numbered
     * ascending by frequency, per the caller's input) */
    ...
    /* O(σ log σ), lengths[i]: Length of codeword for symbol i (symbols are
     * numbered ascending by frequency, per the caller's input) */
...
stupidedi_huffman_sizeof(const stupidedi_huffman_t* h)
{
    ...
    if (h->type == WAVELET)
        return sizeof(*h) +
            stupidedi_packed_sizeof(h->wavelet.nodes) +
            stupidedi_packed_sizeof(h->wavelet.leaves) +
            stupidedi_wavelet_sizeof(h->wavelet.lengths);

    return sizeof(*h) +
        stupidedi_packed_sizeof(h->packed.encode) +
        stupidedi_packed_sizeof(h->packed.decode) +
        stupidedi_packed_sizeof(h->packed.lengths) +
        stupidedi_packed_sizeof(h->packed.offsets) +
        stupidedi_packed_sizeof(h->packed.symbols);
}
...
stupidedi_huffman_encode_wavelet(const stupidedi_huffman_t* h, const size_t i, uint64_t* w)
{
    ...
    const uint8_t length = l;

    /* rank() counts occurrences strictly before i (0-indexed), but r[x] below
     * is the 1-indexed rank among same-depth reversed path labels -- so this
     * needs the occurrence count *through* i, i.e. rank(...)+1. */
    size_t r;
    r = stupidedi_wavelet_rank(h->wavelet.lengths, l, i) + 1;
    ...
    for (; l > 0; --l)
    {
        /* Child level is l, parent level is l-1. wavelet.c reads codewords
         * with the shallowest level (nearest the root) as the MSB and the
         * deepest level (nearest the leaf) as the LSB, so each bit is placed
         * at its final position directly -- a progressive shift would put
         * the bits in the opposite (leaf-to-root) order instead. */
...
stupidedi_huffman_decode_wavelet(const stupidedi_huffman_t* h, const uint64_t bits, const uint8_t length, size_t* i)
{
    ...
    for (size_t l = 0; l < length; ++l)
    {
        ...
        {
            if (i != NULL)
                *i = stupidedi_wavelet_select(h->wavelet.lengths, r, l + 1) - 1;
            return l + 1;
        }
...
init_generate_codewords_packed(stupidedi_huffman_t* h, const size_t L, const size_t* c)
{
    ...
    for (size_t l = 1, i = 0; l <= max_l ; ++l)
    {
        ...
        /* Codewords still in the queue after the last level (max_l) are
         * unused slack from filling the tree (Kraft sum < 1 can leave unused
         * space at the deepest level too, since init_fill_unused_space only
         * redistributes levels 1..max_l-1). Growing them by another bit
         * would need max_l+1 bits, one more than the ringbuf's fixed L-bit
         * width can hold -- and there's no next level to use them for. */
        if (l < max_l)
        {
            const size_t qsize
...
init_generate_codewords_wavelet(stupidedi_huffman_t* h, size_t L, const size_t* c)
{
    ...
    nodes   = stupidedi_packed_new(max_l + 1, nbits(count + 1));
    leaves  = stupidedi_packed_new(max_l + 1, nbits(count + 1));
    lengths = stupidedi_packed_new(h->count, nbits(max_l + 1));
    ...
    /* lengths[i] must hold the codeword length of symbol i directly (no
     * reversal at read time, unlike packed.lengths), since encode_wavelet
     * calls access(lengths, i) with the raw symbol id. Symbols are numbered
     * ascending by frequency, and codewords are assigned shortest-first as l
     * increases, so the k-th codeword assigned overall (k=0 shortest) belongs
     * to the (count-k-1)-th symbol (the most frequent remaining). */
    for (size_t l = 1, k = 0; l <= max_l ; ++l)
    {
        stupidedi_packed_write(leaves, l, c[l]);
        stupidedi_packed_write(nodes,  l, qsize);

        for (size_t n = 0; n < c[l]; ++n, ++k)
            stupidedi_packed_write(lengths, count - k - 1, l);
