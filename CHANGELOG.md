## gh-65
  ...
  * Fix performance issues #65
  * Fix issue caused when delimiters are control characters #189

  **Renamed**

  * `Stupidedi::Reader::Separators.empty` is renamed to `.blank`
  * `Stupidedi::Reader::SimpleElementTok` is renamed to `Stupidedi::Tokens::SimpleElementTok`
  * `Stupidedi::Reader::CompositeElementTok` is renamed to `Stupidedi::Tokens::CompositeElementTok`
  * `Stupidedi::Reader::ComponentElementTok` is renamed to `Stupidedi::Tokens::ComponentElementTok`
  * `Stupidedi::Interchanges::ElementTypes` is renamed to `Stupidedi::Interchanges::Common::ElementTypes`

  **Added**

  * `Stupidedi::Reader::Tokenizer`
  * `Stupidedi::Reader::Tokenizer::Result`
  * `Stupidedi::Reader::Input`
  * `Stupidedi::Position`
  * `Stupidedi::Position::NoPosition`
  * `Stupidedi::Position::OffsetPosition`
  * `Stupidedi::Position::StacktracePosition`
  * `Stupidedi::Tokens::IgnoredTok`
  * `String.match?` added via refinment to backport feature from newer versions of Ruby
  * `Regexp.match?` added via refinment to backport feature from newer versions of Ruby

  Added @private API
  * `Stupidedi::Reader::NativeExt`
  * `Stupidedi::Reader::Slice`
  * `Stupidedi::Reader::Substring`

  **Changed**

  * `String#at(n)` returns nil instead of "" when n is out of bounds
  * `Module.def_delegators` now takes optional parameters type: and instance: which are used
    to generate argument signatures that match the target method (to avoid allocating an
    arary via splat args)

  **Removed**

  * `Object.bind` refinement (deprecated now that Object#then has been standardized)
  * `Stupidedi::Exceptions::TokenizeError`
  * `Stupidedi::Reader::Result`
  * `Stupidedi::Reader::TokenReader`
  ...
  * `Stupidedi::Reader::SimpleElementTok#remainder`
  * `Stupidedi::Reader::ComponentElementTok#remainder`
  * `Stupidedi::Reader::CompositeElementTok#remainder`
  ...
  * `Stupidedi::Parser::IdentifierStack#pop`
