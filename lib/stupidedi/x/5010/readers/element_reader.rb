  ...
  module FiftyTen
    module Readers
      ...
      class SimpleElementReader < ElementReader
        ...
        def initialize(input, interchange_header, element_def)
          @input, @interchange_header, @element_def =
            input, interchange_header, element_def
        end
      ...
      private
        ...
        def advance(n)
          unless @input.defined_at?(n-1)
            ...
            self.class.new(@input.drop(n), @interchange_header, @element_def)
          end
        end
      end
      ...
      class CompositeElementReader < ElementReader
        ...
        def read_composite_element
          read_components.map do |result|
            result.map{|cs| Values::CompositeElementVal.new(@element_def, cs) }
          end
        end
      ...
      protected
        ...
        def read_components
          head, *tail = @element_def.component_element_uses
            ...
            r.remainder.read_delimiter.flatmap do |d|
              ...
              when @interchange_header.component_separator
                if tail.empty?
                  ...
                  rest = self.class.from_reader(d.remainder, @element_def.tail)
              ...
              when @interchange_header.element_separator,
                ...
                result(r.value.cons(tail.map{|x| x.element_def.empty }),
                       r.remainder)
              end
            end
          end
      ...
      class << CompositeElementReader
        def from_reader(reader, element_def)
          new(reader.input, reader.interchange_header, element_def)
        end
      end
