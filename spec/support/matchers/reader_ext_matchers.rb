module ReaderExtMatchers
  ...
  def be_whitespace(encoding: nil)
    BeWhitespace.new(encoding)
  end
  ...
  class BeWhitespace
    ...
    def matches?(actual)
      @failures = encode(actual).chars.reject{|c| Stupidedi::Reader::NativeExt.whitespace?(c) }
      @failures.empty?
    end
    ...
    def failure_message
      failures = @failures.map{|c| "0x%02x%s" % [c.ord, inspect_char(c)] }.join(", ")
      encoding = @actual.encoding.name
      "expected #{failures} to be recognized as #{encoding} whitespace"
    end

    def does_not_match?(actual)
      @failures = encode(actual).chars.select{|c| Stupidedi::Reader::NativeExt.whitespace?(c) }
      @failures.empty?
    end
    ...
    def failure_message_when_negated
      failures = @failures.map{|c| "0x%02x%s" % [c.ord, inspect_char(c)] }.join(", ")
      encoding = @actual.encoding.name
      "did not expect #{failures} to be recognized as #{encoding} whitespace"
    end

    def description
      if @encoding
        "be recognized as #{@encoding} whitespace"
      else
        "be recognized as whitespace"
      end
    end
